...
class inferThread(threading.Thread):
    def __init__(self, yolov7_wrapper, image_path_batch,name):
        threading.Thread.__init__(self)
    ...
    def run(self):
        ...
        print('input->{}, time->{:.2f}ms, saving into output/ ----- ThreadName {}'.format(self.image_path_batch, use_time * 1000, self.name))
...
if __name__ == "__main__":
    ...
    try:
        ...
        thread10 = warmUpThread(yolov7_wrapper)
        thread10.start()
        # thread1.join()
        for batch in image_path_batches:
            # create a new thread to do inference
            thread1 = inferThread(yolov7_wrapper, batch,1)
            thread2 = inferThread(yolov7_wrapper, batch,2)
            thread3 = inferThread(yolov7_wrapper, batch,3)
            thread4 = inferThread(yolov7_wrapper, batch,4)
            thread5 = inferThread(yolov7_wrapper, batch,5)
            thread1.start()
            thread2.start()
            thread3.start()
            thread4.start()
